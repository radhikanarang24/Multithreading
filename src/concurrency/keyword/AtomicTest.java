    ...
    public static void main(String[] args) {
        Thread countChangerAtomic1 = new Thread(new Runnable() {
            ...
            public void run() {
                while(counter.get()<10) {
                    System.out.println("The value of counter is : " + (counter.addAndGet(1)) +
                            " for thread Name " + Thread.currentThread().getName() + " at time " +
        ...
        Thread countChangerAtomic2 = new Thread(new Runnable() {
            ...
            public void run() {
                while(counter.get()<10) {
                    System.out.println("The value of counter is : " + counter.addAndGet(1) +
                            " for thread Name " + Thread.currentThread().getName() + " at time " + System.currentTimeMillis());
                    try {
                        Thread.sleep(500);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
        ...
        Thread countChangerNormal1 = new Thread(new Runnable() {
            ...
            public void run() {
                while(counterNormal<10) {
                    counterNormal++;
                    System.out.println("++ The value of counter is : " + (counterNormal) + " for thread Name "
                            + Thread.currentThread().getName() + " at time " + System.currentTimeMillis());
                    try {
                        Thread.sleep(500);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
